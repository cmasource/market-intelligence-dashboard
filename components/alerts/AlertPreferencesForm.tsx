...
export function AlertPreferencesForm() {
  ...
  async function save() {
    ...
    try {
      await saveAlertPreferences(userId, { ...preferences, whatsappEnabled: false, whatsappPhoneE164: null });
      setPreferences((current) => ({ ...current, whatsappEnabled: false, whatsappPhoneE164: null }));
      setStatus(language === "es" ? "Preferencias guardadas." : "Preferences saved.");
    }
  ...
  return <AppShell>{loading ? <p className="cma-panel p-6 text-sm">{language === "es" ? "Cargando configuración…" : "Loading settings…"}</p> : <div className="mx-auto max-w-4xl space-y-5">
    ...
    <button type="button" disabled={saving || Boolean(preferences.quietHoursStart) !== Boolean(preferences.quietHoursEnd)} onClick={() => void save()} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-cyan-300/35 bg-cyan-300/10 px-5 text-sm font-semibold text-cyan-100 disabled:opacity-40"><Save size={16} />{saving ? (language === "es" ? "Guardando…" : "Saving…") : (language === "es" ? "Guardar preferencias" : "Save preferences")}</button>
  </div>}</AppShell>;
}
